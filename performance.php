...
$sql_performance = "SELECT * FROM performance WHERE Employee_id = $employee_id";
$sql_address = "SELECT * FROM address WHERE Employee_id = $employee_id";
$performance_result = mysqli_query($conn, $sql_performance);
$performance_array = array(); 
while ($row = mysqli_fetch_assoc($performance_result)) {
  $performance_row = array(); 
  foreach ($row as $column1 => $column2) {
    if($column1 === 'id' or $column1==='Employee_id' )continue;
      $performance_row[$column1] = $column2;
  }
  $performance_array[] = $performance_row; 
}
 mysqli_close($conn);
?>
<!DOCTYPE html>
<html>
  ...
  <body>
    ...
    <div class="container">
      ...
      <?php 
      foreach ($performance_array as $attendance_row) {

...
$sql_salary = "SELECT * FROM salary WHERE Employee_id = $employee_id";
$salary_result = mysqli_query($conn, $sql_salary);
$salary_array = array(); 
while ($row = mysqli_fetch_assoc($salary_result)) {
  $salary_row = array(); 
  foreach ($row as $column1 => $column2) {
    if($column1 === 'id' or $column1==='Employee_id')continue;
      $salary_row[$column1] = $column2;
  }
  $salary_array[] = $salary_row; 
}
 mysqli_close($conn);
?>
<!DOCTYPE html>
<html>
  ...
  <body>
    ...
    <div class="container">
      ...
      <?php 
        foreach ($salary_array as $about_row) {
